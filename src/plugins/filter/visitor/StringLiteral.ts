 ...
 * @LastEditTime: 2023-10-31 11:41:39
...
export default function(path:any) {
  ...
  if(baseUtils.hasChineseSymbols(value) && baseUtils.checkAgainstRegexArray(value, [])) {
    ...
    if (types.isJSXAttribute(parent)) {
      ...
      let keyValue = baseUtils.generateId(value);
      let replaceNode = baseUtils.createI18nTranslator(value, true, keyValue);
      path.replaceWith(replaceNode);
    } else {
      let replaceNode = baseUtils.createI18nTranslator(value, true);
      path.replaceWith(replaceNode);
    }
  }
}
